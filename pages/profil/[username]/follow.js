...
export default function username({connected,user,target}) {
  ...
  return(
    <Skeleton connected={connected} user={user}>
      <ProfilFollow user={user} target={target}/>
    </Skeleton>
  )
}
...
export async function getServerSideProps({req,params}) {
  ...
  if(req.cookies.auth){
    ...
    if(payload){
      connected = true;
      let userQuery = await User.findOne({email: payload.username}).populate('follows.user')
      let followers = await User.find({"follows":{user:userQuery}})
      user = {
